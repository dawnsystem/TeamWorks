    ...
    try {
      ...
      switch (action.type) {
        ...
        case 'create_with_subtasks':
          result = await executeCreateWithSubtasks(action, userId, prisma);
          break;
        ...
        case 'delete_bulk':
          result = await executeDeleteBulk(action, userId, prisma);
          break;
        
        case 'move_bulk':
          result = await executeMoveBulk(action, userId, prisma);
          break;
        
        case 'reorder':
          result = await executeReorder(action, userId, prisma);
          break;
...
/**
 * Execute create task with subtasks (recursive)
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function executeCreateWithSubtasks(action: AIAction, userId: string, prisma: any) {
  if (action.entity !== 'task' || !action.data) {
    return null;
  }

  const data = action.data as any;
  
  // Helper function to create task recursively
  const createTaskRecursive = async (taskData: any, parentId: string | null = null): Promise<any> => {
    const inboxProject = await prisma.projects.findFirst({
      where: {
        nombre: 'Inbox',
        ...projectAccessQuery(userId),
    ...
    const resolvedProject = targetProject ?? inboxProject;
    if (!resolvedProject) {
      throw new Error('No se encontró un proyecto válido');
    }
    ...
    let labelConnections: any = undefined;
    ...
    const task = await prisma.tasks.create({
      data: {
        titulo: taskData.titulo,
        descripcion: taskData.descripcion || null,
        prioridad: taskData.prioridad || null,
        fechaVencimiento,
        projectId: resolvedProject.id,
        sectionId: targetSectionId,
        parentId,
        completada: false,
        orden: 0,
        ...(labelConnections && { task_labels: labelConnections }),
      },
    });

    // Create subtasks recursively if they exist
  ...
  // Create the main task and its subtasks
  const mainTask = await createTaskRecursive(data, null);
  return mainTask;
}

/**
 * Execute delete bulk operation
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function executeDeleteBulk(action: AIAction, userId: string, prisma: any) {
  if (action.entity !== 'task') return null;
  ...
  if (data?.filter) {
    ...
    if (filter.projectName) {
      const project = await prisma.projects.findFirst({
        where: {
          nombre: { equals: filter.projectName, mode: 'insensitive' },
          ...projectAccessQuery(userId),
        },
      });
      if (project) where.projectId = project.id;
    ...
    if (filter.sectionName && where.projectId) {
      const section = await prisma.sections.findFirst({
        where: {
          projectId: where.projectId,
          nombre: { equals: filter.sectionName, mode: 'insensitive' },
        },
      });
      if (section) where.sectionId = section.id;
    }

    if (filter.completada !== undefined) {
      where.completada = filter.completada;
    }

    if (filter.prioridad) {
      where.prioridad = filter.prioridad;
    }
  }

  // Execute bulk delete
  const result = await prisma.tasks.deleteMany({ where });
  return { deletedCount: result.count };
}

/**
 * Execute move bulk operation
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function executeMoveBulk(action: AIAction, userId: string, prisma: any) {
  if (action.entity !== 'task') return null;
  ...
  if (data?.filter) {
    ...
    if (filter.labelName) {
      const label = await prisma.labels.findFirst({
        where: {
          nombre: { equals: filter.labelName, mode: 'insensitive' },
          userId,
        },
      });
      if (label) {
        where.task_labels = {
          some: { labelId: label.id },
        };
      }
    }
  }

  // Find target project - support both flat and nested structure
  let targetProjectId = null;
  const targetProjectName = data?.targetProjectName || data?.target?.projectName;
  if (targetProjectName) {
    const targetProject = await prisma.projects.findFirst({
      where: {
        nombre: { equals: targetProjectName, mode: 'insensitive' },
        ...projectAccessQuery(userId),
      },
    });
    if (targetProject) targetProjectId = targetProject.id;
  }

  // Find target section if specified - support both flat and nested structure
  let targetSectionId = null;
  const targetSectionName = data?.targetSectionName || data?.target?.sectionName;
  if (targetSectionName && targetProjectId) {
    const targetSection = await prisma.sections.findFirst({
      where: {
        projectId: targetProjectId,
        nombre: { equals: targetSectionName, mode: 'insensitive' },
      },
    });
    if (targetSection) targetSectionId = targetSection.id;
  }

  // Build update data
  const updateData: any = {};
  if (targetProjectId) updateData.projectId = targetProjectId;
  if (targetSectionId !== null) updateData.sectionId = targetSectionId;
...
async function executeReorder(action: AIAction, userId: string, prisma: any) {
  ...
  const taskToMove = await prisma.tasks.findFirst({
    where: {
      titulo: { contains: data.taskTitle, mode: 'insensitive' },
      projects: projectAccessQuery(userId),
    },
  });
